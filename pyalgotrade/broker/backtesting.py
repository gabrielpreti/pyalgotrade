...
class Broker(broker.BaseBrokerImpl):
    """Backtesting broker.
    ...
    def commitOrderExecution(self, order, dateTime, fillInfo):
        ...
        cost, sharesDelta, resultingCash = self.calculateCostSharesDeltaAndResultingCash(order, price, quantity,
                                                                                         commission)

        # Check that we're ok on cash after the commission.
        if resultingCash >= 0 or self._allowNegativeCash:
            self.handleOrderExecution(order, dateTime, price, quantity, commission, resultingCash, sharesDelta)
    ...
    def __preProcessOrder(self, order, bar_):
        ...
        if self.isOrderExpired(order, bar_):
            ret = False
    ...
    def __processOrder(self, order, bar_):
        ...
        if order.isActive():
            self._postProcessOrder(order, bar_)
    ...
    def __onBarsImpl(self, order, bars):
        ...
        if bar_ is not None:
            ...
            self.acceptOrder(bar_.getDateTime(), order)

            if order.isActive():
                # This may trigger orders to be added/removed from _activeOrders.
    ...
    def onBars(self, dateTime, bars):
        ...
        ordersToProcess = self.getActiveOrders()
